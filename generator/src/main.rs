...
fn main() -> std::io::Result<()> {
    ...
    let wc3_dest: SocketAddr = SocketAddr::from((broadcast_ip, WARCRAFT3_PORT));
    let wc3_socket: UdpSocket = UdpSocket::bind(SocketAddr::from((local_ip, WARCRAFT3_PORT)))?;
    wc3_socket.set_broadcast(true)?;

    let tl2_dest: SocketAddr = SocketAddr::from((broadcast_ip, TORCHLIGHT2_PORT));
    let tl2_socket: UdpSocket = UdpSocket::bind(SocketAddr::from((local_ip, TORCHLIGHT2_PORT)))?;
    ...
    loop {
        ...
        // println!("Sent {} bytes.", _tl2_bytes_sent);
